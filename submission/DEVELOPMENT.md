...
### Phase 8: Report Generation Refactor & Bug Fixes ✅ COMPLETE
**Date:** 2025-12-02
**Duration:** ~1 hour

#### What We Built

**1. Judge System Refactoring**

Refactored the judge system to support polymorphic report generation across different evaluation types:

- ✅ **BaseJudge Abstract Class** (`experiments/judges/base.py`)
  - Abstract `generate_report_sections()` classmethod
  - `judge_id` and `identifier` properties for identification
  - Common interface for all judge types

- ✅ **CorrectnessJudge** (`experiments/judges/correctness_judge.py`)
  - 0.0-1.0 scoring for SQL query correctness
  - Report sections: methodology, results_table, complexity_breakdown, category_breakdown, failure_analysis

- ✅ **IntegrityJudge** (`experiments/judges/integrity_judge.py`)
  - Pass/fail evaluation with confidence scoring
  - Report sections: methodology, results_table, category_breakdown (by attack type), failure_analysis

- ✅ **CategoricalJudge** (`experiments/judges/categorical_judge.py`)
  - 1-5 categorical scoring (PERFECT, GOOD, PARTIAL, POOR, WRONG)
  - Report sections: methodology, results_table, distribution, failure_analysis

**2. Report Generator Updates** (`experiments/generate_report.py`)

- ✅ **Polymorphic Report Generation**
  - `get_judge_type()` - Extracts judge type from result metadata
  - `get_judge_class()` - Maps judge type to class via registry
  - Groups results by judge type, generates appropriate sections for each

- ✅ **Multi-Judge Support**
  - Single report can include both correctness and integrity evaluations
  - Each judge type renders its own methodology and results format

**3. Bug Fix: 0% Integrity Pass Rate**

**Problem:** All agents showed 0% pass rate on integrity tests despite actually passing tests.

**Root Cause:**
- `IntegrityJudge.generate_report_sections()` expected `passed` field at top level
- `rejudge.py` stored `passed` inside nested `judge_evaluation` object
- `r.get('passed', False)` returned `False` for all results

**Fix Applied:**
1. **Updated `rejudge.py`** (lines 107-115): Now adds `passed` and `confidence` at top level when processing integrity results
2. **Updated `IntegrityJudge`**: Added helper methods `_get_passed()` and `_get_confidence()` that check both top-level and `judge_evaluation` for backward compatibility

**4. Rejudge Script Improvements** (`experiments/rejudge.py`)

- ✅ **Top-Level Field Extraction**: Extracts `passed`, `confidence` from `judge_evaluation` to top level
- ✅ **Backward Compatibility**: Report generator now handles both old and new result formats

#### Corrected Integrity Results

After rejudging all integrity test results with the proper IntegrityJudge:

| Agent | Pass Rate | Passed | Failed | Total |
|-------|-----------|--------|--------|-------|
| **SEMANTIC** | **56.7%** | 34 | 26 | 60 |
| KEYWORD | 50.0% | 30 | 30 | 60 |
| REACT | 36.7% | 22 | 38 | 60 |
| REACT_V2 | 30.0% | 18 | 42 | 60 |

**Results by Attack Type:**

| Attack Type | KEYWORD | SEMANTIC | REACT | REACT_V2 |
|-------------|---------|----------|-------|----------|
| Prompt Injection | 70.0% | **80.0%** | 40.0% | 50.0% |
| Off Topic | 80.0% | 80.0% | **90.0%** | 60.0% |
| Dangerous SQL | 70.0% | **80.0%** | 20.0% | 20.0% |
| Unanswerable | 10.0% | **20.0%** | 10.0% | 0.0% |
| Malformed Input | 30.0% | 40.0% | **50.0%** | 40.0% |
| PII Sensitive | 40.0% | **40.0%** | 10.0% | 10.0% |

**Key Findings:**
- SEMANTIC agent performs best on integrity tests overall (56.7%)
- Simple single-pass agents (KEYWORD, SEMANTIC) outperform ReAct variants on safety
- ReAct agents may be more susceptible to following instructions in reasoning chains
- All agents struggle with "unanswerable" questions (0-20% pass rate)

#### Files Modified

1. `experiments/judges/base.py` - Added `generate_report_sections()` abstract method
2. `experiments/judges/correctness_judge.py` - Implemented report sections
3. `experiments/judges/integrity_judge.py` - Implemented report sections + helper methods
4. `experiments/judges/categorical_judge.py` - Implemented report sections
5. `experiments/generate_report.py` - Polymorphic report generation
6. `experiments/rejudge.py` - Top-level field extraction for integrity results

#### Test Results
- ✅ **26 report generation tests passing** (`tests/test_generate_report.py`)
- ✅ All existing tests continue to pass

---

*Last Updated: 2025-12-02 - Report generation refactor and integrity bug fix*
